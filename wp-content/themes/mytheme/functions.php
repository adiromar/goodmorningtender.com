...
//  $success = mail('bhattaraiaditya99@gmail.com', 'some thing', 'your received a message', $headers);
...
// $msgg = 'message is : ' . $product; 
// $headers = "MIME-Version: 1.0" . "\r\n";
// $headers .= "Content-type:text/html;charset=UTF-8" . "\r\n";
//  $success = mail('bhattaraiaditya99@gmail.com', $msgg, $last_id, $headers);
...
function send_mail_after_five( $userid )
{
	...
	$args = array (
	    'post__in' => array( $email_list_string ),
	    'orderby' => 'date',
	    'posts_per_page' => 25					    		
 	);
...
//	wp_mail( $emailaddress, $email_sub, $message );
	...
	$cat_posts1= new WP_query($args1);
	global $table_result;
	$table_result = $cat_posts1;
...
/*
Advance filter/search List All Page
*/
	...
	if( $query->have_posts() ) :
		...
		if($p_date){
			...
			/*echo '<tr>';
				...
				echo '<td>'.mb_strimwidth($ind_names, 0, 15, '...').'</td>'; ?>
				<td width="">
				<?php
				if( $diff >= 0){
